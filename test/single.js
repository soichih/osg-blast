...
var job = workflow.submit({
    ...
    rundir: function(rundir_path, done_prepare) {
        async.series([
            //symlink blast executable
            function(next) {
                which("blastn", function(err, path) {
                    if(err) {
                        done("can't find blast executable:blastn");
                    } else {
                        fs.symlink(path, rundir_path+'/blastn', next);
                    }
                });
            },
            ...
            function(next) {
                fs.open(rundir_path+'/params.sh', 'w', function(err, fd) {
                    ...
                    fs.writeSync(fd, "export oasis_dbpath=/cvmfs/oasis.opensciencegrid.org/osg/projects/IU-GALAXY/blastdb/human_genomic.1-22-2014\n");
                    fs.writeSync(fd, "export dbname=human_genomic.00\n");
                    fs.writeSync(fd, "export blast=blastn\n");
                    fs.writeSync(fd, "export blast_opts=\"-evalue 0.001 -best_hit_score_edge 0.05 -outfmt 6 -best_hit_overhang 0.25 -perc_identity 98.0\"\n");
